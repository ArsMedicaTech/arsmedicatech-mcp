...
from .mcp_api_tools import mcp  # type: ignore
...
async def call_bayesian_model_tool(
    ...
    data: Dict[str, Any] = Field(
        description="Dictionary containing the model input data"
    ),
...
async def predict_sepsis_tool(
    ...
    systolic_bp: Optional[int] = Field(
        default=None, description="Optional systolic blood pressure in mmHg"
    ),
    diastolic_bp: Optional[int] = Field(
        default=None, description="Optional diastolic blood pressure in mmHg"
    ),
    respiratory_rate: Optional[int] = Field(
        default=None, description="Optional respiratory rate in breaths per minute"
    ),
    ctx: Optional[Context] = None,
) -> Dict[str, Any]:
    """
...
@mcp.tool
async def get_available_models_tool(ctx: Optional[Context] = None) -> Dict[str, Any]:
    """
...
async def batch_predict_sepsis_tool(
    patient_data: List[Dict[str, Any]] = Field(
        description="List of dictionaries containing patient data"
    ),
    ctx: Optional[Context] = None,
) -> Dict[str, Any]:
    """
