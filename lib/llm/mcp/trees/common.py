"""
Common utilities for working with decision trees.
"""
...
register_compare_op(
    "regex", lambda x, pattern: re.fullmatch(pattern, str(x)) is not None
)
...
def _choose_branch(
    branches: Dict[BranchKey, Any], arg: Any, subterm: str, path: list[str]
...
def decision_tree_lookup(tree: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
    ...
    return {"decision": decision, "reason": reason, "path_taken": path_taken}
