...
def test_medgemma_tools():
    ...
    result = analyze_medical_image(test_image, view="PA", prompt="Describe any abnormalities")
    ...
    derm_result = analyze_medical_image(derm_image, view="DermCloseUp", prompt="Assess the lesion")
    ...
    if derm_result.get('status') == 'success':
        ...
        if 'findings' in findings:
            ...
            if 'lesion' in lesion_findings:
                print(f"   Lesion status: {lesion_findings['lesion'].get('status', 'N/A')}")
    print()
    
    # Test 4: Pathology image analysis
    print("4. Testing pathology image analysis:")
    path_image = "https://example.com/pathology_slide.tiff"
    path_result = analyze_medical_image(path_image, view="Other", prompt="Analyze cellular patterns")
